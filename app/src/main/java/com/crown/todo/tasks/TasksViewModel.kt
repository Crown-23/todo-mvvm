...
/**
 * Description：ViewModel for the task list screen.
 */
class TasksViewModel(
    ...
    fun setFiltering(requestType: TasksFilterType) {
        ...
        when (requestType) {
            TasksFilterType.ALL_TASKS -> setFilter(
                R.string.label_all,
                R.string.no_tasks_all,
                R.drawable.ic_no_fill
            )
            TasksFilterType.ACTIVE_TASKS -> setFilter(
                R.string.label_active,
                R.string.no_tasks_active,
                R.drawable.ic_check_circle_96dp
            )
            TasksFilterType.COMPLETED_TASKS -> setFilter(
    ...
    fun clearCompletedTasks() = viewModelScope.launch {
        // TODO: [hub] _forceUpdate未赋值，列表为什么会自动刷新
